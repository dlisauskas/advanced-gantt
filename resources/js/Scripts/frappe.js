...
export default class Gantt {
    ...
    make_grid_background() {
        ...
        const distinct_rows = [...new Set(this.tasks.map((x) => x.group))];
        const grid_height =
        ...
        document.querySelector(".gantt-container").style.height =
            grid_height + "px";
    }
    ...
    update_button_position() {
        ...
        if (
            ...
            this.$side_header.style.top = `${containerRect.scrollTop + buttonRect.top
                }px`;
        } else {
    ...
    make_grid_ticks() {
        ...
        let tick_height =
            (this.options.bar_height + this.options.padding) * this.groups.length;
    ...
    make_bars() {
        const focusedTasks = this.tasks.filter((task) => task.focused);
        const normalTasks = this.tasks.filter((task) => !task.focused);

        const sortedTasks = [...normalTasks, ...focusedTasks];
        this.bars = sortedTasks.map((task) => {
